...
<#assign messagesQ = "SELECT id, subject, conversation.solved, kudos.sum(weight), author.login, author.view_href, author.avatar, conversation.last_post_time, post_time, post_time_friendly, metrics.views, view_href, replies.count(*), board.id, board.view_href, board.title FROM messages WHERE depth=0 ${levelQuery} ${interactionQ} ORDER BY conversation.last_post_time DESC LIMIT ${numRows}" />
...
<div class="latest_author">
    ${text.format("latest_posts.latest_author")}&nbsp<a href="${latestAuthor.view_href}" title="${latestAuthor.login?html}" alt="${latestAuthor.login?html}">${latestAuthor.login?html}</a>
</div>
</#if>
        </#if>
...
<div class="wrapper">
    ...
    <div class="replies">
        <a href="${message.view_href}" title="${message.subject?html}" alt="${message.subject?html}">
            ...
            <span class="unit">${message.replies.count}&nbsp;${text.format("messages.column.thread-replies-count.label", message.replies.count)}</span></a>
    </div>
</div>
