<#assign label = coreNode.title />

<div class="wrapper center-content">
    <h2>Nieuwe berichten in ${label}</h2>
    <ul class="inline-grid desktop-one-third mobile-one-second">
        <li class="grid-item doelgroep-nav-item">
            <a href="/t5/Forum/ct-p/healthy_living">
                <div class="item-content">
                    <div class="avatar-wrapper">
                        <div class="author-avatar">
                            <img src="${asset.get("/html/assets/avatar.png")}" alt="Gijsbertus">
                        </div>
                    </div>
                    <div class="item-header">Topic door <strong>Gijsbertus</strong></div>
                    <div class="item-footer">Eigen risico bij erfelijkheidsonderzoek</div>
                </div>
            </a>
        </li>
        <li class="grid-item doelgroep-nav-item">
            <a href="/t5/Denk-Mee/idb-p/IdeaExchange">
                <div class="item-content">
                    <div class="avatar-wrapper">
                        <div class="author-avatar">
                            <img src="${asset.get("/html/assets/avatar.png")}" alt="Janniedegroot">
                        </div>
                    </div>
                    <div class="item-header">Idee door <strong>Janniedegroot</strong></div>
                    <div class="item-footer">Diabetes dienst met gratis koolhydraatteller</div>
                </div>
            </a>
        </li>
        <li class="grid-item doelgroep-nav-item">
            <a href="/t5/Blog/bg-p/Blog">
                <div class="item-content">
                    <div class="avatar-wrapper">
                        <div class="author-avatar">
                            <img src="${asset.get("/html/assets/avatar.png")}" alt="Gijsbertus">
                        </div>
                    </div>
                    <div class="item-header">Blog door <strong>Gijsbertus</strong></div>
                    <div class="item-footer">Zelf crackers en vruchtensapjes kunnen leiden</div>
                </div>
            </a>
        </li>
    </ul>
</div>